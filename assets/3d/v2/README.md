...
`studio.glb` 为 438,286 个三角面、332 个网格、13,849,916 字节（约 13.21 MiB）。本轮优先检验美术质量；相比 v1 体积与几何量增加，尚未完成正式性能验收、纹理压缩或 LOD。网格数不等于实际渲染调用数。

## 坐标、动作与使用

单位米，Y 向上。员工局部面向 +Z；桌子使用侧为 +Z，员工放在桌子 +Z 侧并绕 Y 旋转 π 面向桌面。家具可独立加载。

`console.glb` 与 `studio.glb` 必须保留包含机器人几何体的独立 `Coordinator` 组，客户端使用它驱动机器人动作并挂载思考气泡。总控台静态家具先合并网格，再添加独立合并的机器人；不能将机器人一起压入 `Console` 的静态网格。该结构由资产校验与 `node scripts/check-coordinator-bubble.cjs` 的真实 GLB 浏览器回归共同检查。
